...
import { exportHRTransferData, importFromExcel } from "@/lib/excelUtils";
...
export default function HumanCapitalBudgetPage() {
  ...
  const handleImport = (file: File) => {
    importFromExcel<Record<string, unknown>>(file, (data) => {
      const imported: HRTransferItem[] = data.map(row => ({
        id: crypto.randomUUID(),
        positionName: (row["Position"] as string) || "",
        libraryItemId: "",
        department: (row["Department"] as DepartmentName) || "IT",
        quantity: Number(row["Quantity"]) || 1,
        unitCost: Number(row["Unit Cost (ETB)"]) || 0,
        totalCost: Number(row["Total Cost (ETB)"]) || 0,
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => exportHRTransferData(items)} className="gap-1">
